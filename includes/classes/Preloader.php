...
use function SwiftPress\Utils\site_cache_dir;
use const SwiftPress\Constants\DEFERRED_PRELOAD_QUEUE_CRON_NAME;
...
class Preloader {
	...
	public function setup() {
		...
		add_action( 'admin_post_swiftpress_preload_cache', [ $this, 'start_preload' ] );
		add_action( 'swiftpress_purge_all_cache', [ $this, 'setup_preload_queue' ] );
		add_action( 'swiftpress_clean_site_cache_dir', [ $this, 'setup_preload_queue' ] );
		add_action( 'swiftpress_advanced_cache_purge_post', [ $this, 'deferred_preload_queue' ], 10, 2 );
		add_action( 'swiftpress_expired_files_deleted', [ $this, 'add_expired_urls_to_preload_queue' ], 10, 2 );
	...
	public function admin_bar_menu( $wp_admin_bar ) {
		...
		$wp_admin_bar->add_menu(
			array(
				'id'     => 'preload-cache',
				'title'  => __( 'Preload Cache', 'swiftpress' ),
				'href'   => wp_nonce_url( admin_url( 'admin-post.php?action=swiftpress_preload_cache' ), 'swiftpress_preload_cache' ),
				'parent' => 'swiftpress',
			)
		);
	}
	...
	public function start_preload() {
		...
		if ( SWIFTPRESS_IS_NETWORK && ! current_user_can( 'manage_network' ) ) {
			$redirect_url = add_query_arg( 'sp_action', 'start_preload_err_permission', wp_get_referer() );
		...
		\SwiftPress\Utils\log( sprintf( 'Preload triggered from admin bar.' ) );

		$this->setup_preload_queue();

		$redirect_url = add_query_arg( 'sp_action', 'start_preload', wp_get_referer() );
		wp_safe_redirect( esc_url_raw( $redirect_url ) );
		exit;
	}
	...
	public function setup_preload_queue() {
		...
		if ( SWIFTPRESS_IS_NETWORK ) {
			...
			foreach ( $sites as $site_id ) {
				...
				\SwiftPress\Utils\log( sprintf( 'Populating preload queue for site: %d', $site_id ) );
		...
		// Sitemap preloading: populate DB table from detected sitemap
		if ( ! empty( $this->settings['enable_sitemap_preload'] ) ) {
			SitemapPreloader::factory()->populate_from_sitemap();
		}
	}
	...
	protected function populate_preload_queue() {
		...
		if ( $this->settings['preload_homepage'] ) {
			...
			if ( ! empty( $front_page ) ) {
				...
				\SwiftPress\Utils\log( sprintf( 'Front page URL added to preload queue: %s', $front_page_url ) );
			}
			...
			if ( ! empty( $posts_page ) ) {
				...
				\SwiftPress\Utils\log( sprintf( 'Posts Page URL added to preload queue: %s', $posts_page_url ) );
			}
			...
			\SwiftPress\Utils\log( sprintf( 'Home URL added to preload queue: %s', $home_url ) );
		}
		...
		if ( $this->settings['preload_public_posts'] ) {
			...
			\SwiftPress\Utils\log( sprintf( 'Public posts added to preload queue. ' ) );
		}
		...
		if ( $this->settings['preload_public_tax'] ) {
			...
			\SwiftPress\Utils\log( sprintf( 'Public tax terms added to preload queue. ' ) );
		}
	...
	public function deferred_preload_queue( $post_id, $urls ) {
		\SwiftPress\Utils\log( sprintf( 'Post ID %d purged from cache, adding related URLs to preload queue with a delay.', $post_id ) );
	...
	public function add_expired_urls_to_preload_queue( $expired_files ) {
		 ...
		 *
		 * @hook   swiftpress_preload_expired_urls
		 *
		 ...
		 * @since  2.0
		 */
		$preload_expired_urls = apply_filters( 'swiftpress_preload_expired_urls', true );
	...
	public function prepare_public_posts_urls() {
		 ...
		 *
		 * @hook   swiftpress_preload_public_posts_offset
		 *
		 ...
		 * @since  2.0
		 */
		$offset           = (int) apply_filters( 'swiftpress_preload_public_posts_offset', 0 );
		$max_preload_item = $this->preload_max_post_count();

		\SwiftPress\Utils\log( sprintf( 'Adding public posts to queue' ) );
		\SwiftPress\Utils\log( sprintf( 'OFFSET: %s', $offset ) );
		\SwiftPress\Utils\log( sprintf( 'LIMIT: %s', $max_preload_item ) );

		/**
		 * Filters public post types.
		 *
		 * @hook   swiftpress_preload_post_types
		 *
		...
		\SwiftPress\Utils\log( sprintf( 'Found posts: %s', count( $posts ) ) );
	...
	public function prepare_public_tax_terms_urls() {
		 ...
		 *
		 * @hook   swiftpress_preload_taxonomies
		 *
		 ...
		 * @since  1.0
		 */
		$taxonomies = apply_filters( 'swiftpress_preload_taxonomies', $taxonomies );

		foreach ( $taxonomies as $tax ) {
			\SwiftPress\Utils\log( sprintf( 'Taxonomy added to queue: %s', $tax ) );

			/**
			 * Filters taxonomy offset for preload.
			 *
			 * @hook   swiftpress_preload_public_taxonomies_offset
			 *
			 ...
			 * @since  2.0
			 */
			$offset = (int) apply_filters( 'swiftpress_preload_public_taxonomies_offset', 0 );
			$limit  = $this->preload_max_term_count();

			/**
			 * Filters term query args.
			 *
			 * @hook   swiftpress_preload_tax_term_args
			 *
			 ...
			 */
			$args = apply_filters(
				'swiftpress_preload_tax_term_args',
				[
					'hide_empty' => false,
					'orderby'    => 'count',
					'offset'     => $offset,
			...
			\SwiftPress\Utils\log( 'Taxonomy preload args: [swiftpress_preload_tax_term_args]:' . print_r( $args, true ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
	...
	public function preload_max_post_count() {
		 ...
		 *
		 * @hook   swiftpress_preload_post_limit
		 *
		 ...
		 * @since  2.0
		 */
		return apply_filters( 'swiftpress_preload_post_limit', 1000 );
	}
	...
	public function preload_max_term_count() {
		 ...
		 *
		 * @hook   swiftpress_preload_term_limit
		 *
		 ...
		 * @since  2.0
		 */
		return apply_filters( 'swiftpress_preload_term_limit', 100 );
	}
	...
	public function get_prefetch_dns() {
		 ...
		 * @since  2.0
		 */
		return apply_filters( 'swiftpress_prefetch_dns', $prefetch_dns );
	}
	...
	public function get_preconnect_resources() {
		 ...
		 *
		 * @hook   swiftpress_preconnect_resource
		 *
		 ...
		 * @since  2.2
		 */
		return apply_filters( 'swiftpress_preconnect_resource', $preconnect_resources );
	}
	...
	public static function preload_request( $url, $args = [] ) {
		 ...
		 *
		 * @hook   swiftpress_preload_url_request_args
		 *
		...
		$request_args = apply_filters(
			'swiftpress_preload_url_request_args',
			[
				'timeout'    => 0.01,
				'blocking'   => false,
				'user-agent' => 'SwiftPress Preloader',
				'sslverify'  => false,
			]
		);
		 ...
		 *
		 * @hook  swiftpress_preload_http_request
		 *
		 ...
		 * @since 2.0
		 */
		do_action( 'swiftpress_preload_http_request', $url, $args );

		\SwiftPress\Utils\log( sprintf( 'Processing..: %s', $url ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_print_r
	...
	public static function wait( $delay = 500000 ) {
		 ...
		 *
		 * @hook   swiftpress_preload_request_interval
		 *
		 * @param  {int} $delay Delay time in microseconds.
		 *
		 * @return {int} New value.
		 * @since  2.0
		 */
		$delay = absint( apply_filters( 'swiftpress_preload_request_interval', $delay ) );
	...
	public static function mobile_user_agent() {
		 ...
		 *
		 * @hook   swiftpress_preload_mobile_user_agent
		 *
		 ...
		 * @since  2.0
		 */
		return apply_filters( 'swiftpress_preload_mobile_user_agent', 'SwiftPress Preloader mobile iPhone' );
	}
	...
	protected function add_url_to_preload_queue( $url ) {
		 ...
		 *
		 * @hook   swiftpress_preload_add_url_to_queue
		 *
		 ...
		 * @since  3.4
		 */
		$do_preload = apply_filters( 'swiftpress_preload_add_url_to_queue', true, $url );

		if ( $do_preload ) {
			$this->get_preloader()->push_to_queue( $url );
			$this->queue_dirty = true;
			\SwiftPress\Utils\log( sprintf( 'URL added to preload queue    : %s', $url ) );
		} else {
			\SwiftPress\Utils\log( sprintf( 'URL skipped from preload queue: %s', $url ) );
		}
	}
	...
	public function dispatch_preload_queue() {
		...
		\SwiftPress\Utils\log( 'Dispatched preload queue.' );
	}
