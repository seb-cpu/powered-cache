...
class CSS extends WP_Styles {
	...
	public function do_items( $handles = false, $group = false ) {
		...
		foreach ( $this->to_do as $key => $handle ) {
			 ...
			 *
			 * @hook   swiftpress_fo_css_do_concat
			 *
			 ...
			 * @since  2.0
			 */
			$do_concat = apply_filters( 'swiftpress_fo_css_do_concat', $do_concat, $handle, $css_url );
		...
		foreach ( $stylesheets as $idx => $stylesheets_group ) {
			foreach ( $stylesheets_group as $media => $css ) {
				 ...
				 *
				 * @hook   swiftpress_fo_style_loader_tag
				 *
				 ...
				 * @since  2.0
				 */
				echo apply_filters( 'swiftpress_fo_style_loader_tag', $css_tag, $handles, $href, $media ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
